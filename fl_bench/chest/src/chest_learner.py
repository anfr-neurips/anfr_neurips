...
from fl_bench.utils.misc import extract_weights, load_weights, seed_everything

from .dm import ChestDataModule
from .trainers import get_trainer
from .validator import Validator


class ChestLearner(Learner):
    def __init__(
        self,
    ...
    def initialize(self, parts, fl_ctx):
        ...
        data_dir = os.getenv("CHEST_ROOT")

        self.dm = ChestDataModule(
            data_dir=data_dir,
            client_idx=fl_ctx.get_identity_name(),
            central=self.central,
            batch_size=config_train["batch_size"],
            cache_rate=config_train["cache_rate"],
            seed=self.seed,
        )

        self.class_weights = torch.FloatTensor(self.dm.weights).to(self.device)
        # initialize bias of classification layer
        # to the log of the inverse class frequency
        if "nf" in config_train["model_name"]:
            self.model.head.fc.bias.data = -torch.log(self.class_weights)
        else:
            self.model.fc.bias.data = -torch.log(self.class_weights)
        self.local_steps = config_train["local_steps"]
        self.key_metric = "mean_auroc"
        self.best_metric = 0.0
        self.best_model_path = "models/best_model.pt"
        self.last_model_path = "models/last.pt"
        self.trainer = (
            get_trainer(self.method, config_train, self.class_weights, model=self.model)
            if self.method == "scaffold"
            else get_trainer(self.method, config_train, self.class_weights)
        )
    ...
    def train(self, data, fl_ctx, abort_signal) -> Shareable:
        ...
        for i in range(self.max_retry + 1):
            try:
                # pass reference to writer so we can add the pr curve
                metrics = self.validator.run(
                    self.model, self.dm.val_dataloader, 
                    logger=self.writer, step=self.trainer.current_step
                )
                break
            except Exception as e:
                if i < self.max_retry:
        ...
        for m, v in metrics.items():
            if m == 'optimal_thresholds':
                continue
            table.add_row([m, v])
            self.writer.add_scalar(f"{m}/Post-Train Val {m}", v, self.trainer.current_step)
    ...
    def validate(self, data, fl_ctx, abort_signal):
        ...
        for i in range(self.max_retry+1):
            try:
                ...
                if i < self.max_retry:
                    self.log_warning(fl_ctx, "Error encountered in validation, retrying.")
    ...
    def get_model_for_validation(self, model_name: str, fl_ctx: FLContext) -> Shareable:
        ...
        try:
            if model_name == ModelName.FINAL_MODEL:
                ...
                self.log_info(fl_ctx, f"Load last model from {self.last_model_path}")
            elif model_name == ModelName.BEST_MODEL:
        ...
        if model_data:
            ...
            if model_name == ModelName.BEST_MODEL:
                ...
                self.log_error(fl_ctx, f"last local model not available at {self.last_model_path}")
