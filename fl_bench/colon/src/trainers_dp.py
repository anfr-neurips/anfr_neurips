...
class BaseTrainer(FLComponent):
    ...
    def run(
        ...
        data_loader = privacy_engine._prepare_data_loader(
        ...
        self.opt = privacy_engine._prepare_optimizer(
            optimizer=self.opt,
...
def get_trainer(method, config, **kwargs):
    if method in ["fedavg", "fedprox", "fedopt"]:
        ...
        return PartPrivateTrainer(config, private_layers=["attn", "se"])
    elif method == "fedper":
