...
from .dm import ColonDataModule
from fl_bench.utils.misc import load_weights, extract_weights, seed_everything
from .trainers_dp import get_trainer
from .validator import Validator
from fl_bench.networks.colon_nets import *


class ColonLearner(Learner):
    def __init__(
        self,
    ...
    def initialize(self, parts, fl_ctx):
        ...
        self.privacy_engine = opacus.privacy_engine.PrivacyEngine()
        self.model = eval(
            config_train["model_name"] + f"({self.seed})"
        ).to(self.device)

        self.model = self.privacy_engine._prepare_model(module=self.model)

        data_dir = os.getenv("COLON_ROOT")

        self.dm = ColonDataModule(
            data_dir=data_dir,
            idx_root=config_train["train_idx_root"],
            client_idx=fl_ctx.get_identity_name(),
            central=self.central,
            batch_size=config_train["batch_size"],
            seed=self.seed,
        )
        self.dsize = self.dm.train_dataset.__len__()
        self.delta = 0.1 / self.dsize
        self.log_info(fl_ctx, f"Train dataset size: {self.dsize}. Setting delta to {self.delta}")
    ...
    def train(self, data, fl_ctx, abort_signal) -> Shareable:
        ...
        if self.method == "scaffold":
            ...
            dxo_global_ctrl_weights = dxo_collection.data.get(
            ...
            self.trainer.scaffold_helper.load_global_controls(
            ...
            try:
                ...
                if i < self.max_retry:
                    ...
                    if os.path.exists(self.last_model_path):
                        self.log_warning(
                            fl_ctx, f"Loading last model from {self.last_model_path}"
                        )
                        self.trainer.load_checkpoint(
                            self.last_model_path, self.model, fl_ctx=fl_ctx
                        )
                        load_weights(self.model._module, global_weights)
        ...
        for i in range(self.max_retry + 1):
            try:
                ...
                if i < self.max_retry:
                    self.log_warning(
                        fl_ctx,
                        f"Something wrong in val, retrying ({i+1}/{self.max_retry}).",
                    )
                    # Reset dataset & dataloader
                    self.dm.teardown()
                    self.dm.setup()
        ...
        for m, v in metrics.items():
            ...
            self.writer.add_scalar(
                "Accuracy/Post-Train Val Accuracy", v, self.trainer.current_step
            )
        self.log_info(fl_ctx, str(table))

        epsilon = self.privacy_engine.get_epsilon(delta=self.delta)

        self.log_info(fl_ctx, f"Privacy Engine Epsilon: {epsilon}")
        ...
        local_weights = extract_weights(self.model._module)
        weight_diff = {}
    ...
    def validate(self, data, fl_ctx, abort_signal):
        ...
        load_weights(self.model._module, dxo.data)
